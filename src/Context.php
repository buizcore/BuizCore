...
class Context
{
  ...
  /**
   * Fields für Formvalidierung
   * @var array
   */
  public $fields = [];

  /**
   * Validierte Field Daten
   * @var array
   */
  public $fieldsData = [];

  /**
   * Soweit vorhanden die haupt entity
   * @var Entity
   */
  public $entity = null;
...
/*////////////////////////////////////////////////////////////////////////////*/
   ...
   */
  public function __construct($request = null, $env = null)
  {
  ...
  public function __get($key)
  {
    ...
    */
    ...
    {
        // flags per default
        $this->flags = new TFlag($request->data('flags',Validator::BOOLEAN));
    ...
    */
    public function validateFields($request)
    {
      ...
      foreach ($this->fields as $key => $fields) {
          ...
          if ($key == '@def') {
              foreach($fields as $subKey => $fieldData){
                  $this->fieldsData[$key][$subKey] = $request->data($subKey, $fieldData[0] );
                  
                  if (is_null($this->fieldsData[$key][$subKey]) && !is_null($fieldData[1])) {
                      $this->fieldsData[$key][$subKey] = $fieldData[1];
                  }
              }
          } else {
              foreach($fields as $subKey => $fieldData){
                  $this->fieldsData[$key][$subKey] = $request->data($key, $fieldData[0], $subKey);
    ...
    * @return []
    */
    public function getSaveFields($key)
    {
    
      return isset($this->saveFields[$key])?$this->saveFields[$key]:[];
    ...
    * @return NULL
    */
    public function field($subKey, $key='@def')
    {
        return isset($this->fieldsData[$key][$subKey])?$this->fieldsData[$key][$subKey]:null;
  ...
  /**
   * This function ist for the narf...
   * 
  ...
  public function narf()
  {
    ...
    * @param unknown $error
    */
    public function dataIsEmpty($key, $errorMsg)
    {
        ...
        if (is_array($key)) {
          ...
          switch ($numDepth) {
              ...
              default: {
                  throw new InvalidArgumentException('Invalid amount of params: '.$numDepth);
                  return false;
              }
          }
    ...
    { 
        return !emnpty($this->actualErrors);
    }// public function hasErrors */
    ...
    public function handleErrors() 
    {
        ...
        throw new InvalidRequest_Exception(implode(','.NL, $this->actualErrors));
    }//end public function handleErrors */
