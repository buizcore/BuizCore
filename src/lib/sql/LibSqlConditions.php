...
///////////////////////////////////////s//////////////////////////////////////*/
   ...
   * @param array $conditions
   */
  public function inject($criteria, $fields, $conditions = [])
  {
  ...
  protected function handleCondition_Text($cond)
  {
    ...
    switch ($cond->cond) {
      ...
      case ESearchText::EQUALS: {
        ...
        if($cond->cs)
          $sql .= ' '.$fieldName[2]." = '".$this->db->escape($cond->value)."'";
        else
          $sql .= ' UPPER('.$fieldName[2].") = UPPER('".$this->db->escape($cond->value)."')";
      ...
      case ESearchText::START_WITH: {
        ...
        if($cond->cs)
          $sql .= ' '.$fieldName[2]." like '".$this->db->escape($cond->value)."%'";
        else
          $sql .= ' UPPER('.$fieldName[2].") like UPPER('".$this->db->escape($cond->value)."%')";
