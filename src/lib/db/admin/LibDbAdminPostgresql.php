...
SQL;
    ...
    foreach ($data as $row) {
      ...
      if(in_array($type, array('text','char','varchar'))){

          if (trim($row[LibDbAdmin::COL_DEFAULT]) != '')
              $sql .= ' DEFAULT \''.$this->db->addSlashes((string)$row[LibDbAdmin::COL_DEFAULT]).'\' ';
...
SQL;
    ...
    * @param string $owner
    */
    public function setViewOwner($viewName, $owner = null)
    {
      
        $this->chownView($viewName, $owner);
    
    }//end public function setViewOwner */
  
  /**
   * @param string $viewName
   */
  public function chownView($viewName, $owner = null)
  {
      $sql ='';
      ...
      if ($owner) {
          ...
          $sql .= "ALTER TABLE {$viewName} OWNER TO {$owner}; ";
   ...
   * @param string $viewName
   */
  public function createReadView($tableName)
  {
  
      /* @var $domainNode DomainNode  */
      $entityName = SParserString::subToCamelCase($tableName).'_Entity';
      
      /* @var $entity Entity   */
      $entity = new $entityName();
  
      $sql = <<<SQL
CREATE VIEW  {$tableName}_r as
SELECT
  * 
FROM
  {$tableName}
...
SQL;
      ...
      if ($entity->trackChanges() || $entity->trackDeletion()) {
        
          $sql .= ' WHERE ';
          
          if ($entity->trackChanges() && $entity->trackDeletion()) {
              $sql .= ' not m_version = '.Db::PRE_VALUE.' AND m_time_deleted is null; ';
          } else if($entity->trackChanges()){
              $sql .= ' not m_version = '.Db::PRE_VALUE.'; ';
          } else {
              $sql .= ' m_time_deleted is null; ';
          }
      }
  ...
  }//end public function createReadView */
